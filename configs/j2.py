...
train_ids = [x for cnt,x in enumerate(train_ids) if x not in bad_ids if cnt<2]
...
max_nchunks = nchunks_per_epoch * 20000

validate_every = int(5000 * nchunks_per_epoch)
save_every = int(5000 * nchunks_per_epoch)
...
def build_model(l_in=None):
    ...
    l_out = nn.layers.DenseLayer(l, num_units=1,
                                 W=nn.init.Constant(0.),b=nn.init.Constant(0.0),
...
def build_objective(model, deterministic=False):
    ...
    objectives = (1 - 2*ten_pt+ ten_pt**2) / (np.float32(np.log(10.))*errors)**2
...
def build_updates(train_loss, model, learning_rate):
    ...
    updates = nn.updates.adam(grad, nn.layers.get_all_params(model.l_out, trainable=True), learning_rate)
    return updates
